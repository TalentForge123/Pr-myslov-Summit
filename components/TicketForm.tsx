...
export default function TicketForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Příklad přesměrování na Stripe Checkout
      // UPOZORNĚNÍ: V produkčním prostředí byste neměli generovat Stripe Checkout URL na klientovi
      // Měli byste to udělat na vašem serveru (API routě v Next.js), aby se zabránilo zneužití.
      // Toto je jen pro ukázku frontendu.

      // const stripeCheckoutUrl = `https://checkout.stripe.com/pay/cs_test_your_stripe_checkout_id#fidkdEMwn8HWqAXF3roLp2k7P3t800xP6C6GgN`; // Změňte na skutečné ID
      // window.location.href = stripeCheckoutUrl;

      // Pro účely testování a ukázky, stačí přesměrovat na hlavní stránku nebo potvrdit
      ...
      <div className="bg-white p-8 rounded-lg shadow-md max-w-md mx-auto">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">
            Děkujeme za vaši objednávku. Nyní byste byli přesměrováni na platební bránu pro dokončení platby.
          </p>
          <div className="mt-6">
            <button
              type="button"
          ...
          {ticketTypes.map((ticket) => (
            <div key={ticket.id} className="relative">
              <div className={`border rounded-lg p-4 ${formData.ticketType === ticket.id ? 'border-blue-500 bg-blue-50' : 'border-gray-300'}`}>
                <label className="flex items-start cursor-pointer"> {/* Přidal jsem cursor-pointer */}
                  <input
                    type="radio"
                    name="ticketType"
                    value={ticket.id}
